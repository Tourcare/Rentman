import { z } from 'zod';
export declare const updating_user_information_mutation: z.ZodObject<{
    created_at: z.ZodString;
    message: z.ZodString;
} & {
    mutation_code: z.ZodLiteral<"updating_user_information">;
    from: z.ZodObject<{
        email_address: z.ZodOptional<z.ZodNullable<z.ZodString>>;
        full_name: z.ZodOptional<z.ZodNullable<z.ZodString>>;
        phone_number: z.ZodOptional<z.ZodNullable<z.ZodOptional<z.ZodEffects<z.ZodString, string, string>>>>;
    }, "strip", z.ZodTypeAny, {
        email_address?: string | null | undefined;
        full_name?: string | null | undefined;
        phone_number?: string | null | undefined;
    }, {
        email_address?: string | null | undefined;
        full_name?: string | null | undefined;
        phone_number?: string | null | undefined;
    }>;
    to: z.ZodObject<{
        email_address: z.ZodOptional<z.ZodNullable<z.ZodString>>;
        full_name: z.ZodOptional<z.ZodNullable<z.ZodString>>;
        phone_number: z.ZodOptional<z.ZodNullable<z.ZodOptional<z.ZodEffects<z.ZodString, string, string>>>>;
    }, "strip", z.ZodTypeAny, {
        email_address?: string | null | undefined;
        full_name?: string | null | undefined;
        phone_number?: string | null | undefined;
    }, {
        email_address?: string | null | undefined;
        full_name?: string | null | undefined;
        phone_number?: string | null | undefined;
    }>;
}, "strip", z.ZodTypeAny, {
    message: string;
    created_at: string;
    mutation_code: "updating_user_information";
    from: {
        email_address?: string | null | undefined;
        full_name?: string | null | undefined;
        phone_number?: string | null | undefined;
    };
    to: {
        email_address?: string | null | undefined;
        full_name?: string | null | undefined;
        phone_number?: string | null | undefined;
    };
}, {
    message: string;
    created_at: string;
    mutation_code: "updating_user_information";
    from: {
        email_address?: string | null | undefined;
        full_name?: string | null | undefined;
        phone_number?: string | null | undefined;
    };
    to: {
        email_address?: string | null | undefined;
        full_name?: string | null | undefined;
        phone_number?: string | null | undefined;
    };
}>;
export declare const acs_user_pending_mutations: z.ZodDiscriminatedUnion<"mutation_code", [z.ZodObject<{
    created_at: z.ZodString;
    message: z.ZodString;
} & {
    mutation_code: z.ZodLiteral<"creating">;
}, "strip", z.ZodTypeAny, {
    message: string;
    created_at: string;
    mutation_code: "creating";
}, {
    message: string;
    created_at: string;
    mutation_code: "creating";
}>, z.ZodObject<{
    created_at: z.ZodString;
    message: z.ZodString;
} & {
    mutation_code: z.ZodLiteral<"deleting">;
}, "strip", z.ZodTypeAny, {
    message: string;
    created_at: string;
    mutation_code: "deleting";
}, {
    message: string;
    created_at: string;
    mutation_code: "deleting";
}>, z.ZodObject<{
    created_at: z.ZodString;
    message: z.ZodString;
} & {
    mutation_code: z.ZodLiteral<"deferring_creation">;
    scheduled_at: z.ZodNullable<z.ZodOptional<z.ZodString>>;
}, "strip", z.ZodTypeAny, {
    message: string;
    created_at: string;
    mutation_code: "deferring_creation";
    scheduled_at?: string | null | undefined;
}, {
    message: string;
    created_at: string;
    mutation_code: "deferring_creation";
    scheduled_at?: string | null | undefined;
}>, z.ZodObject<{
    created_at: z.ZodString;
    message: z.ZodString;
} & {
    mutation_code: z.ZodLiteral<"updating_user_information">;
    from: z.ZodObject<{
        email_address: z.ZodOptional<z.ZodNullable<z.ZodString>>;
        full_name: z.ZodOptional<z.ZodNullable<z.ZodString>>;
        phone_number: z.ZodOptional<z.ZodNullable<z.ZodOptional<z.ZodEffects<z.ZodString, string, string>>>>;
    }, "strip", z.ZodTypeAny, {
        email_address?: string | null | undefined;
        full_name?: string | null | undefined;
        phone_number?: string | null | undefined;
    }, {
        email_address?: string | null | undefined;
        full_name?: string | null | undefined;
        phone_number?: string | null | undefined;
    }>;
    to: z.ZodObject<{
        email_address: z.ZodOptional<z.ZodNullable<z.ZodString>>;
        full_name: z.ZodOptional<z.ZodNullable<z.ZodString>>;
        phone_number: z.ZodOptional<z.ZodNullable<z.ZodOptional<z.ZodEffects<z.ZodString, string, string>>>>;
    }, "strip", z.ZodTypeAny, {
        email_address?: string | null | undefined;
        full_name?: string | null | undefined;
        phone_number?: string | null | undefined;
    }, {
        email_address?: string | null | undefined;
        full_name?: string | null | undefined;
        phone_number?: string | null | undefined;
    }>;
}, "strip", z.ZodTypeAny, {
    message: string;
    created_at: string;
    mutation_code: "updating_user_information";
    from: {
        email_address?: string | null | undefined;
        full_name?: string | null | undefined;
        phone_number?: string | null | undefined;
    };
    to: {
        email_address?: string | null | undefined;
        full_name?: string | null | undefined;
        phone_number?: string | null | undefined;
    };
}, {
    message: string;
    created_at: string;
    mutation_code: "updating_user_information";
    from: {
        email_address?: string | null | undefined;
        full_name?: string | null | undefined;
        phone_number?: string | null | undefined;
    };
    to: {
        email_address?: string | null | undefined;
        full_name?: string | null | undefined;
        phone_number?: string | null | undefined;
    };
}>, z.ZodObject<{
    created_at: z.ZodString;
    message: z.ZodString;
} & {
    mutation_code: z.ZodLiteral<"updating_access_schedule">;
    from: z.ZodObject<{
        starts_at: z.ZodNullable<z.ZodString>;
        ends_at: z.ZodNullable<z.ZodString>;
    }, "strip", z.ZodTypeAny, {
        starts_at: string | null;
        ends_at: string | null;
    }, {
        starts_at: string | null;
        ends_at: string | null;
    }>;
    to: z.ZodObject<{
        starts_at: z.ZodNullable<z.ZodString>;
        ends_at: z.ZodNullable<z.ZodString>;
    }, "strip", z.ZodTypeAny, {
        starts_at: string | null;
        ends_at: string | null;
    }, {
        starts_at: string | null;
        ends_at: string | null;
    }>;
}, "strip", z.ZodTypeAny, {
    message: string;
    created_at: string;
    mutation_code: "updating_access_schedule";
    from: {
        starts_at: string | null;
        ends_at: string | null;
    };
    to: {
        starts_at: string | null;
        ends_at: string | null;
    };
}, {
    message: string;
    created_at: string;
    mutation_code: "updating_access_schedule";
    from: {
        starts_at: string | null;
        ends_at: string | null;
    };
    to: {
        starts_at: string | null;
        ends_at: string | null;
    };
}>, z.ZodObject<{
    created_at: z.ZodString;
    message: z.ZodString;
} & {
    mutation_code: z.ZodLiteral<"updating_suspension_state">;
    from: z.ZodObject<{
        is_suspended: z.ZodBoolean;
    }, "strip", z.ZodTypeAny, {
        is_suspended: boolean;
    }, {
        is_suspended: boolean;
    }>;
    to: z.ZodObject<{
        is_suspended: z.ZodBoolean;
    }, "strip", z.ZodTypeAny, {
        is_suspended: boolean;
    }, {
        is_suspended: boolean;
    }>;
}, "strip", z.ZodTypeAny, {
    message: string;
    created_at: string;
    mutation_code: "updating_suspension_state";
    from: {
        is_suspended: boolean;
    };
    to: {
        is_suspended: boolean;
    };
}, {
    message: string;
    created_at: string;
    mutation_code: "updating_suspension_state";
    from: {
        is_suspended: boolean;
    };
    to: {
        is_suspended: boolean;
    };
}>, z.ZodObject<{
    created_at: z.ZodString;
    message: z.ZodString;
} & {
    mutation_code: z.ZodLiteral<"updating_group_membership">;
    from: z.ZodObject<{
        acs_access_group_id: z.ZodNullable<z.ZodString>;
    }, "strip", z.ZodTypeAny, {
        acs_access_group_id: string | null;
    }, {
        acs_access_group_id: string | null;
    }>;
    to: z.ZodObject<{
        acs_access_group_id: z.ZodNullable<z.ZodString>;
    }, "strip", z.ZodTypeAny, {
        acs_access_group_id: string | null;
    }, {
        acs_access_group_id: string | null;
    }>;
}, "strip", z.ZodTypeAny, {
    message: string;
    created_at: string;
    mutation_code: "updating_group_membership";
    from: {
        acs_access_group_id: string | null;
    };
    to: {
        acs_access_group_id: string | null;
    };
}, {
    message: string;
    created_at: string;
    mutation_code: "updating_group_membership";
    from: {
        acs_access_group_id: string | null;
    };
    to: {
        acs_access_group_id: string | null;
    };
}>]>;
export type AcsUserPendingMutation = z.infer<typeof acs_user_pending_mutations>;
declare const _acs_user_pending_mutations_map: z.ZodObject<{
    creating: z.ZodNullable<z.ZodOptional<z.ZodObject<{
        created_at: z.ZodString;
        message: z.ZodString;
    } & {
        mutation_code: z.ZodLiteral<"creating">;
    }, "strip", z.ZodTypeAny, {
        message: string;
        created_at: string;
        mutation_code: "creating";
    }, {
        message: string;
        created_at: string;
        mutation_code: "creating";
    }>>>;
    deleting: z.ZodNullable<z.ZodOptional<z.ZodObject<{
        created_at: z.ZodString;
        message: z.ZodString;
    } & {
        mutation_code: z.ZodLiteral<"deleting">;
    }, "strip", z.ZodTypeAny, {
        message: string;
        created_at: string;
        mutation_code: "deleting";
    }, {
        message: string;
        created_at: string;
        mutation_code: "deleting";
    }>>>;
    deferring_creation: z.ZodNullable<z.ZodOptional<z.ZodObject<{
        created_at: z.ZodString;
        message: z.ZodString;
    } & {
        mutation_code: z.ZodLiteral<"deferring_creation">;
        scheduled_at: z.ZodNullable<z.ZodOptional<z.ZodString>>;
    }, "strip", z.ZodTypeAny, {
        message: string;
        created_at: string;
        mutation_code: "deferring_creation";
        scheduled_at?: string | null | undefined;
    }, {
        message: string;
        created_at: string;
        mutation_code: "deferring_creation";
        scheduled_at?: string | null | undefined;
    }>>>;
    updating_access_schedule: z.ZodNullable<z.ZodOptional<z.ZodObject<{
        created_at: z.ZodString;
        message: z.ZodString;
    } & {
        mutation_code: z.ZodLiteral<"updating_access_schedule">;
        from: z.ZodObject<{
            starts_at: z.ZodNullable<z.ZodString>;
            ends_at: z.ZodNullable<z.ZodString>;
        }, "strip", z.ZodTypeAny, {
            starts_at: string | null;
            ends_at: string | null;
        }, {
            starts_at: string | null;
            ends_at: string | null;
        }>;
        to: z.ZodObject<{
            starts_at: z.ZodNullable<z.ZodString>;
            ends_at: z.ZodNullable<z.ZodString>;
        }, "strip", z.ZodTypeAny, {
            starts_at: string | null;
            ends_at: string | null;
        }, {
            starts_at: string | null;
            ends_at: string | null;
        }>;
    }, "strip", z.ZodTypeAny, {
        message: string;
        created_at: string;
        mutation_code: "updating_access_schedule";
        from: {
            starts_at: string | null;
            ends_at: string | null;
        };
        to: {
            starts_at: string | null;
            ends_at: string | null;
        };
    }, {
        message: string;
        created_at: string;
        mutation_code: "updating_access_schedule";
        from: {
            starts_at: string | null;
            ends_at: string | null;
        };
        to: {
            starts_at: string | null;
            ends_at: string | null;
        };
    }>>>;
    updating_group_membership: z.ZodNullable<z.ZodOptional<z.ZodRecord<z.ZodString, z.ZodObject<{
        created_at: z.ZodString;
        message: z.ZodString;
    } & {
        mutation_code: z.ZodLiteral<"updating_group_membership">;
        from: z.ZodObject<{
            acs_access_group_id: z.ZodNullable<z.ZodString>;
        }, "strip", z.ZodTypeAny, {
            acs_access_group_id: string | null;
        }, {
            acs_access_group_id: string | null;
        }>;
        to: z.ZodObject<{
            acs_access_group_id: z.ZodNullable<z.ZodString>;
        }, "strip", z.ZodTypeAny, {
            acs_access_group_id: string | null;
        }, {
            acs_access_group_id: string | null;
        }>;
    }, "strip", z.ZodTypeAny, {
        message: string;
        created_at: string;
        mutation_code: "updating_group_membership";
        from: {
            acs_access_group_id: string | null;
        };
        to: {
            acs_access_group_id: string | null;
        };
    }, {
        message: string;
        created_at: string;
        mutation_code: "updating_group_membership";
        from: {
            acs_access_group_id: string | null;
        };
        to: {
            acs_access_group_id: string | null;
        };
    }>>>>;
    updating_suspension_state: z.ZodNullable<z.ZodOptional<z.ZodObject<{
        created_at: z.ZodString;
        message: z.ZodString;
    } & {
        mutation_code: z.ZodLiteral<"updating_suspension_state">;
        from: z.ZodObject<{
            is_suspended: z.ZodBoolean;
        }, "strip", z.ZodTypeAny, {
            is_suspended: boolean;
        }, {
            is_suspended: boolean;
        }>;
        to: z.ZodObject<{
            is_suspended: z.ZodBoolean;
        }, "strip", z.ZodTypeAny, {
            is_suspended: boolean;
        }, {
            is_suspended: boolean;
        }>;
    }, "strip", z.ZodTypeAny, {
        message: string;
        created_at: string;
        mutation_code: "updating_suspension_state";
        from: {
            is_suspended: boolean;
        };
        to: {
            is_suspended: boolean;
        };
    }, {
        message: string;
        created_at: string;
        mutation_code: "updating_suspension_state";
        from: {
            is_suspended: boolean;
        };
        to: {
            is_suspended: boolean;
        };
    }>>>;
    'updating_user_information.full_name': z.ZodNullable<z.ZodOptional<z.ZodObject<{
        created_at: z.ZodString;
        message: z.ZodString;
    } & {
        mutation_code: z.ZodLiteral<"updating_user_information">;
        from: z.ZodObject<{
            full_name: z.ZodNullable<z.ZodString>;
        }, "strip", z.ZodTypeAny, {
            full_name: string | null;
        }, {
            full_name: string | null;
        }>;
        to: z.ZodObject<{
            full_name: z.ZodNullable<z.ZodString>;
        }, "strip", z.ZodTypeAny, {
            full_name: string | null;
        }, {
            full_name: string | null;
        }>;
    }, "strip", z.ZodTypeAny, {
        message: string;
        created_at: string;
        mutation_code: "updating_user_information";
        from: {
            full_name: string | null;
        };
        to: {
            full_name: string | null;
        };
    }, {
        message: string;
        created_at: string;
        mutation_code: "updating_user_information";
        from: {
            full_name: string | null;
        };
        to: {
            full_name: string | null;
        };
    }>>>;
    'updating_user_information.email_address': z.ZodNullable<z.ZodOptional<z.ZodObject<{
        created_at: z.ZodString;
        message: z.ZodString;
    } & {
        mutation_code: z.ZodLiteral<"updating_user_information">;
        from: z.ZodObject<{
            email_address: z.ZodNullable<z.ZodString>;
        }, "strip", z.ZodTypeAny, {
            email_address: string | null;
        }, {
            email_address: string | null;
        }>;
        to: z.ZodObject<{
            email_address: z.ZodNullable<z.ZodString>;
        }, "strip", z.ZodTypeAny, {
            email_address: string | null;
        }, {
            email_address: string | null;
        }>;
    }, "strip", z.ZodTypeAny, {
        message: string;
        created_at: string;
        mutation_code: "updating_user_information";
        from: {
            email_address: string | null;
        };
        to: {
            email_address: string | null;
        };
    }, {
        message: string;
        created_at: string;
        mutation_code: "updating_user_information";
        from: {
            email_address: string | null;
        };
        to: {
            email_address: string | null;
        };
    }>>>;
    'updating_user_information.phone_number': z.ZodNullable<z.ZodOptional<z.ZodObject<{
        created_at: z.ZodString;
        message: z.ZodString;
    } & {
        mutation_code: z.ZodLiteral<"updating_user_information">;
        from: z.ZodObject<{
            phone_number: z.ZodNullable<z.ZodEffects<z.ZodString, string, string>>;
        }, "strip", z.ZodTypeAny, {
            phone_number: string | null;
        }, {
            phone_number: string | null;
        }>;
        to: z.ZodObject<{
            phone_number: z.ZodNullable<z.ZodEffects<z.ZodString, string, string>>;
        }, "strip", z.ZodTypeAny, {
            phone_number: string | null;
        }, {
            phone_number: string | null;
        }>;
    }, "strip", z.ZodTypeAny, {
        message: string;
        created_at: string;
        mutation_code: "updating_user_information";
        from: {
            phone_number: string | null;
        };
        to: {
            phone_number: string | null;
        };
    }, {
        message: string;
        created_at: string;
        mutation_code: "updating_user_information";
        from: {
            phone_number: string | null;
        };
        to: {
            phone_number: string | null;
        };
    }>>>;
}, "strip", z.ZodTypeAny, {
    creating?: {
        message: string;
        created_at: string;
        mutation_code: "creating";
    } | null | undefined;
    deleting?: {
        message: string;
        created_at: string;
        mutation_code: "deleting";
    } | null | undefined;
    deferring_creation?: {
        message: string;
        created_at: string;
        mutation_code: "deferring_creation";
        scheduled_at?: string | null | undefined;
    } | null | undefined;
    updating_access_schedule?: {
        message: string;
        created_at: string;
        mutation_code: "updating_access_schedule";
        from: {
            starts_at: string | null;
            ends_at: string | null;
        };
        to: {
            starts_at: string | null;
            ends_at: string | null;
        };
    } | null | undefined;
    updating_suspension_state?: {
        message: string;
        created_at: string;
        mutation_code: "updating_suspension_state";
        from: {
            is_suspended: boolean;
        };
        to: {
            is_suspended: boolean;
        };
    } | null | undefined;
    updating_group_membership?: Record<string, {
        message: string;
        created_at: string;
        mutation_code: "updating_group_membership";
        from: {
            acs_access_group_id: string | null;
        };
        to: {
            acs_access_group_id: string | null;
        };
    }> | null | undefined;
    'updating_user_information.full_name'?: {
        message: string;
        created_at: string;
        mutation_code: "updating_user_information";
        from: {
            full_name: string | null;
        };
        to: {
            full_name: string | null;
        };
    } | null | undefined;
    'updating_user_information.email_address'?: {
        message: string;
        created_at: string;
        mutation_code: "updating_user_information";
        from: {
            email_address: string | null;
        };
        to: {
            email_address: string | null;
        };
    } | null | undefined;
    'updating_user_information.phone_number'?: {
        message: string;
        created_at: string;
        mutation_code: "updating_user_information";
        from: {
            phone_number: string | null;
        };
        to: {
            phone_number: string | null;
        };
    } | null | undefined;
}, {
    creating?: {
        message: string;
        created_at: string;
        mutation_code: "creating";
    } | null | undefined;
    deleting?: {
        message: string;
        created_at: string;
        mutation_code: "deleting";
    } | null | undefined;
    deferring_creation?: {
        message: string;
        created_at: string;
        mutation_code: "deferring_creation";
        scheduled_at?: string | null | undefined;
    } | null | undefined;
    updating_access_schedule?: {
        message: string;
        created_at: string;
        mutation_code: "updating_access_schedule";
        from: {
            starts_at: string | null;
            ends_at: string | null;
        };
        to: {
            starts_at: string | null;
            ends_at: string | null;
        };
    } | null | undefined;
    updating_suspension_state?: {
        message: string;
        created_at: string;
        mutation_code: "updating_suspension_state";
        from: {
            is_suspended: boolean;
        };
        to: {
            is_suspended: boolean;
        };
    } | null | undefined;
    updating_group_membership?: Record<string, {
        message: string;
        created_at: string;
        mutation_code: "updating_group_membership";
        from: {
            acs_access_group_id: string | null;
        };
        to: {
            acs_access_group_id: string | null;
        };
    }> | null | undefined;
    'updating_user_information.full_name'?: {
        message: string;
        created_at: string;
        mutation_code: "updating_user_information";
        from: {
            full_name: string | null;
        };
        to: {
            full_name: string | null;
        };
    } | null | undefined;
    'updating_user_information.email_address'?: {
        message: string;
        created_at: string;
        mutation_code: "updating_user_information";
        from: {
            email_address: string | null;
        };
        to: {
            email_address: string | null;
        };
    } | null | undefined;
    'updating_user_information.phone_number'?: {
        message: string;
        created_at: string;
        mutation_code: "updating_user_information";
        from: {
            phone_number: string | null;
        };
        to: {
            phone_number: string | null;
        };
    } | null | undefined;
}>;
export type AcsUserPendingMutationsMap = z.infer<typeof _acs_user_pending_mutations_map>;
export {};
