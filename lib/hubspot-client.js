...
async function getOrder(orderId, properties = []) {
    ...
    const allProps = [...new Set([...defaultProps, ...properties])];
    return getObject(config.hubspot.objectTypes.orders, orderId, allProps);
}
...
async function updateOrder(orderId, properties) {
    return updateObject('orders', orderId, properties);
}

async function deleteOrder(orderId) {
    return deleteObject(config.hubspot.objectTypes.orders, orderId);
}
...
async function addAssociation(fromObjectType, fromId, toObjectType, toId, associationTypeId) {
    const url = `/crm/v3/objects/${fromObjectType}/${fromId}/associations/${toObjectType}/${toId}/${associationTypeId}`;
...
async function getOrderAssociations(orderId, toObjectType) {
    return getAssociations('orders', orderId, toObjectType);
}
