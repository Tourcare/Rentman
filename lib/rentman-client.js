...
const syncLimit = require('./sync-daily-limit');

const logger = createChildLogger('rentman-client');

// =============================================================================
// Rate limit circuit breaker
// Hvis vi rammer for mange 429-svar inden for kort tid, pauser vi alle kald i 12 timer.
// =============================================================================

const CIRCUIT_BREAKER = {
    pausedUntil: null,    // Cache så vi ikke slår op i DB på hvert kald
    lastCheck: 0,
    CHECK_INTERVAL: 30_000,  // Tjek DB maks hvert 30s
    PAUSE_MS: 12 * 60 * 60 * 1000  // 12 timer
};

async function isCircuitOpen() {
    const now = Date.now();

    // Brug cache hvis vi har tjekket for nylig
    if (now - CIRCUIT_BREAKER.lastCheck < CIRCUIT_BREAKER.CHECK_INTERVAL) {
        if (CIRCUIT_BREAKER.pausedUntil && now < new Date(CIRCUIT_BREAKER.pausedUntil).getTime()) {
            return true;
        }
        if (CIRCUIT_BREAKER.pausedUntil && now >= new Date(CIRCUIT_BREAKER.pausedUntil).getTime()) {
            await db.clearCircuitBreaker('rentman');
            CIRCUIT_BREAKER.pausedUntil = null;
            logger.info('CIRCUIT BREAKER: Pause udløbet, genoptager Rentman API kald');
            return false;
        }
        return false;
    }
    ...
    // Slå op i DB
    CIRCUIT_BREAKER.lastCheck = now;
    try {
        const pausedUntil = await db.getCircuitBreaker('rentman');
        CIRCUIT_BREAKER.pausedUntil = pausedUntil;

        if (pausedUntil && now < new Date(pausedUntil).getTime()) {
            return true;
        }
        if (pausedUntil && now >= new Date(pausedUntil).getTime()) {
            await db.clearCircuitBreaker('rentman');
        ...
        logger.warn('Circuit breaker DB check fejlede, fortsætter', { error: err.message });
    }
    return false;
}

async function activateCircuitBreaker() {
    const pausedUntil = new Date(Date.now() + CIRCUIT_BREAKER.PAUSE_MS);
    CIRCUIT_BREAKER.pausedUntil = pausedUntil;
    CIRCUIT_BREAKER.lastCheck = Date.now();
    try {
        await db.setCircuitBreaker('rentman', pausedUntil);
    } catch (err) {
        logger.error('Kunne ikke gemme circuit breaker til DB', { error: err.message });
    }
    const resumeTime = pausedUntil.toLocaleTimeString('da-DK');
    logger.error(`CIRCUIT BREAKER: Daglig API-grænse nået. Pauser ALLE Rentman API kald til ${resumeTime}`, {
...
async function request(method, endpoint, body = null, attempt = 1, { _isRetryQueue = false } = {}) {
    // Daglig sync grænse - kun når sync mode er aktiv
    if (syncLimit.isSyncMode() && attempt === 1) {
        if (!syncLimit.canProceed()) {
            logger.warn('Daglig sync grænse nået, stopper', {
                count: syncLimit.getCount(), limit: syncLimit.DAILY_LIMIT, endpoint
            });
            return { success: false, dailyLimitReached: true, status: 0, data: null };
        }
        syncLimit.increment();
    }

    // Circuit breaker - stop alle kald hvis daglig grænse er ramt
    if (await isCircuitOpen()) {
        const remainMs = new Date(CIRCUIT_BREAKER.pausedUntil).getTime() - Date.now();
        const remainH = Math.round(remainMs / 3600000 * 10) / 10;
        logger.debug(`Circuit breaker aktiv, ${remainH}t tilbage`, { endpoint });
        return { success: false, circuitBreaker: true, status: 429, data: null };
    }
    ...
    try {
        ...
        if (response.status === 429) {
            ...
            // Daglig grænse (50.000 req/dag) - paus alt i 12 timer
            if (errBody.includes('Limit Exceeded')) {
                await activateCircuitBreaker();
                return { success: false, circuitBreaker: true, status: 429, data: null };
            }

            // Normal per-second rate limit - retry med backoff
            const backoffMs = Math.min(1000 * Math.pow(2, attempt - 1), 60000);
            logger.warn(`Rate limited (429), retry ${attempt} efter ${backoffMs}ms`, { endpoint, method });
            await new Promise(res => setTimeout(res, backoffMs));
            return request(method, endpoint, body, attempt + 1, { _isRetryQueue });
        }
        ...
        if (SERVER_RETRY_STATUSES.has(response.status)) {
            ...
            if (attempt <= MAX_IMMEDIATE_RETRIES) {
                ...
                logger.warn(`Server fejl (${response.status}), retry ${attempt}/${MAX_IMMEDIATE_RETRIES} efter ${backoffMs}ms`, { endpoint, method });
                await new Promise(res => setTimeout(res, backoffMs));
                return request(method, endpoint, body, attempt + 1, { _isRetryQueue });
            }
            if (!_isRetryQueue) {
            ...
            return { success: false, queued: !_isRetryQueue, status: response.status, data: null };
        }

        if (response.status === 403 || response.status === 404) {
            return { success: false, notFound: true, status: response.status, data: null };
        }
        ...
        // Netværksfejl (Rentman nede, DNS fejl, timeout osv.)
        if (attempt <= MAX_IMMEDIATE_RETRIES) {
            const backoffMs = Math.min(2000 * Math.pow(2, attempt - 1), 30000);
            logger.warn(`Netværksfejl, retry ${attempt}/${MAX_IMMEDIATE_RETRIES} efter ${backoffMs}ms`, { endpoint, method, error: error.message });
            await new Promise(res => setTimeout(res, backoffMs));
            return request(method, endpoint, body, attempt + 1, { _isRetryQueue });
        }
        if (!_isRetryQueue) {
        ...
        return { success: false, queued: !_isRetryQueue, status: 0, data: null };
    }
}

/**
 * Gemmer et fejlet Rentman request i databasen til senere retry.
 */
async function queueFailedRequest(method, endpoint, body, errorMessage) {
    try {
        await db.insertFailedRequest('rentman', method, endpoint, body, errorMessage);
...
/**
 * Behandler retry-køen for Rentman. Prøver alle gemte requests igen.
 */
async function processRetryQueue() {
    if (await isCircuitOpen()) {
        const remainMs = new Date(CIRCUIT_BREAKER.pausedUntil).getTime() - Date.now();
        const remainH = Math.round(remainMs / 3600000 * 10) / 10;
        logger.info(`Retry-kø: springer over, circuit breaker aktiv (${remainH}t tilbage)`);
        return;
    }

    const failedRequests = await db.getFailedRequests('rentman');
    ...
    for (const item of failedRequests) {
        ...
        const result = await request(item.method, item.endpoint, parsedBody, 1, { _isRetryQueue: true });
...
function startRetryProcessor() {
    ...
    logger.info('Rentman retry-processor startet (kører hvert 10. minut)');
}
